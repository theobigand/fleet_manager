...
class StatsDAO(BaseDAO):
    def get_overview(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'en_sortie': conn.execute(
                "SELECT COUNT(*) FROM vehicules WHERE statut = 'en_sortie'"
            ).fetchone()[0],
            'en_maintenance': conn.execute(
                "SELECT COUNT(*) FROM vehicules WHERE statut = 'en_maintenance'"
            ).fetchone()[0],
            'en_panne': conn.execute(
                "SELECT COUNT(*) FROM vehicules WHERE statut IN ('en_panne', 'immobilise')"
            ).fetchone()[0],
    ...
    def get_usage_by_vehicle(self) -> List[Dict[str, Any]]:
        return self._fetch_all("""
            SELECT 
                ...
                v.marque,
                v.modele,
                v.kilometrage_actuel as km_actuel,
                (
                    SELECT COUNT(*)
                    FROM sorties_reservations
                    WHERE vehicule_id = v.id
                        AND date_sortie_reelle >= DATE('now', '-30 days')
                ) as sorties_30j,
                (
                    SELECT COUNT(DISTINCT date_sortie_reelle)
                    FROM sorties_reservations
                    WHERE vehicule_id = v.id
                        AND date_sortie_reelle >= DATE('now', '-30 days')
                ) as jours_utilises,
    ...
    def get_top_employees(self, limit: int = 10) -> List[Dict[str, Any]]:
        return self._fetch_all("""
            SELECT 
                e.nom,
                e.prenom,
                COUNT(s.id) as nb_sorties,
                COALESCE(SUM(s.km_retour - s.km_depart), 0) as km_total
            FROM employes e
            LEFT JOIN sorties_reservations s 
                ON e.id = s.employe_id 
    ...
    def get_monthly_costs(self, months: int = 6) -> List[Dict[str, Any]]:
        return self._fetch_all("""
            ...
            FROM (
                SELECT date, cout, 'carburant' as type
                FROM ravitaillements
                UNION ALL
                SELECT date, cout, 'maintenance' as type
                FROM maintenances
            )
    ...
    def get_all_echeances(self) -> List[Dict[str, Any]]:
        return self._fetch_all("""
            ...
            FROM employes e
            WHERE e.date_validite_permis IS NOT NULL
                AND e.autorise_conduire = 1
